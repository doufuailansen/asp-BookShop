...
namespace Dao
{
    ...
    public class AccountDao
    {
        ...
        {
            // 查询语句
            _sql = "insert into account (username, password, question, answer, mail, registertime, shopname, shopdescription) values (@username, @password, @question, @answer, @mail,  getdate(),  @shopname, @shopdescription)";
            // 参数列表
            SqlParameter[] parameters =
            {
                ...
                new SqlParameter("@shopdescription",SqlDbType.VarChar)  
            ...
            parameters[5].Value = account.ShopName;
            parameters[6].Value = account.ShopDescription;
        ...
        {
            _sql = "select id, username, password, question, answer, mail, registertime, telephone, address, shopname, shopdescription, shopactivity from account where username = @username and password = @password";
            SqlParameter[] parameters =
            {
                new SqlParameter("@username", SqlDbType.VarChar),
                new SqlParameter("@password", SqlDbType.VarChar)
        ...
        {
            _sql =
                "update account set password = @password, question = @question, answer = @answer, mail = @mail , telephone = @telephone , address = @address where id = @id";
            SqlParameter[] parameters =
            {
        ...
        /// <param name="account"></param>
        /// <returns></returns>
        public bool UpdateShopInfoById(Account account)
        {
            _sql =
                "update account set  mail = @mail , shopActivity = @shopActivity , shopDescription = @shopDescription where id = @id";
            SqlParameter[] parameters =
            {

                new SqlParameter("@mail", account.Mail),
                new SqlParameter("@id", account.Id),
                new SqlParameter("@shopDescription", account.ShopDescription),
                new SqlParameter("@shopActivity", account.ShopActivity)
            };
        ...
        {
            _sql = "select id, username, password, question, answer, mail, registertime, telephone, address, shopname, shopdescription, shopactivity from account";
        ...
        private void LoadEntity(Account account, DataRow dataRow)
        {
            ...
            account.Telephone = dataRow["telephone"].ToString();
            account.Address = dataRow["address"].ToString();
            account.ShopName = dataRow["shopname"].ToString();
            account.ShopDescription = dataRow["shopdescription"].ToString();
            account.ShopActivity = dataRow["shopactivity"].ToString();
        }
    }
