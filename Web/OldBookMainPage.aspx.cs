...
public partial class Web_OldBookMainPage : System.Web.UI.Page
{
    /// <summary>
    /// 热搜图书占位符
    /// </summary>
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        // 热搜图书
        list = oldBookDao.GetAllBooks();
        if (list != null)
        {
            StringBuilder stringBuilder = new StringBuilder();
            foreach (OldBookInfo book in list)
            {
                stringBuilder.AppendFormat("<div class='book'><a href = './Bidding1.aspx?id={0}' ><img src = '{1}' /><h4 class='title' alt='{2}'>{2}</h4><small class='text-info' alt='{3}'>{3} </small><p class='text-danger'>￥{4}</p></a></div>", book.Id, book.Image, book.Name, book.Author, book.OldPrice);
            }
